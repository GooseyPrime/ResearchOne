 ...
 *                          contains "Inc", "Ltd", "University", etc.)
...
function parseAuthor(raw: string): CslAuthor | null {
  ...
  if (/\b(Inc\.?|Ltd\.?|LLC|University|College|Institute|Foundation|Corporation|Company|Group|Council|Society|Association|Department|Office|Bureau|Commission|Agency)\b/i.test(s)) {
    return { literal: s };
  }
