...
describe('GET /admin/cost/summary', () => {
  it('returns aggregated KPI totals', async () => {
    ...
    const app = createApp();
    const res = await request(app).get('/api/admin/cost/summary?days=30').set(adminHeaders());
  ...
  it('returns {available:false, reason:migration_pending} on 42P01', async () => {
    ...
    const app = createApp();
    const res = await request(app).get('/api/admin/cost/summary').set(adminHeaders());
  ...
  it('clamps days to [1, 365]', async () => {
    ...
    const app = createApp();
    const res1 = await request(app).get('/api/admin/cost/summary?days=99999').set(adminHeaders());
    expect(res1.body.days).toBe(365);

    const res2 = await request(app).get('/api/admin/cost/summary?days=0').set(adminHeaders());
    expect(res2.body.days).toBe(1);
  });

  it('rejects non-admin', async () => {
    const app = createApp();
    const res = await request(app).get('/api/admin/cost/summary');
...
describe('GET /admin/cost/timeseries', () => {
  it('returns daily rollup points', async () => {
    ...
    const app = createApp();
    const res = await request(app).get('/api/admin/cost/timeseries?days=7').set(adminHeaders());
  ...
  it('deploy-skew returns empty points array', async () => {
    ...
    const app = createApp();
    const res = await request(app).get('/api/admin/cost/timeseries').set(adminHeaders());
...
describe('GET /admin/cost/breakdown', () => {
  it('returns phase breakdown by default', async () => {
    ...
    const app = createApp();
    const res = await request(app).get('/api/admin/cost/breakdown?days=30').set(adminHeaders());
  ...
  it('accepts dimension=role and dimension=model', async () => {
    ...
    const app = createApp();
    let res = await request(app).get('/api/admin/cost/breakdown?dimension=role').set(adminHeaders());
    expect(res.body.dimension).toBe('agent_role');

    res = await request(app).get('/api/admin/cost/breakdown?dimension=model').set(adminHeaders());
...
describe('GET /admin/cost/reports', () => {
  it('returns per-run rollup rows', async () => {
    ...
    const app = createApp();
    const res = await request(app).get('/api/admin/cost/reports?days=30').set(adminHeaders());
  ...
  it('filters by userId', async () => {
    ...
    const app = createApp();
    await request(app).get('/api/admin/cost/reports?days=30&userId=user_alice').set(adminHeaders());
  ...
  it('filters by phase', async () => {
    ...
    const app = createApp();
    await request(app).get('/api/admin/cost/reports?days=30&phase=Skeptic').set(adminHeaders());
  ...
  it('paginates with limit/offset, clamped', async () => {
    ...
    const app = createApp();
    await request(app).get('/api/admin/cost/reports?limit=9999&offset=-5').set(adminHeaders());
  ...
  it('deploy-skew returns empty rows array', async () => {
    ...
    const app = createApp();
    const res = await request(app).get('/api/admin/cost/reports').set(adminHeaders());
