...
| `DATABASE_ADMIN_URL` | No | **Opt-in:** privileged Postgres connection string (superuser or `CREATEROLE` + DDL rights). If set, the workflow passes it to the VM as **`DATABASE_ADMIN_URL_B64`** (base64); `deploy-runtime.sh` decodes it in-process only — it is **not** appended to logs. Use the same value as in [`docs/RUNBOOKS/application-role-bootstrap.md`](docs/RUNBOOKS/application-role-bootstrap.md). Alternatively, put `DATABASE_ADMIN_URL` only in `backend/.env` on the server and omit this secret. |
