...
│  Tiered defaults (V1) in backend/src/config/index.ts (per-role  │
│  env overrides). Examples: Kimi K2 Thinking + DeepSeek-R1       │
...
# HF repo ids that fail with provider_unavailable will be retried against
# Together. Leave blank to disable the provider-level fallback (recommended
# unless you have a vetted Together account that hosts the same uncensored
# weights — see docs/V2_MODEL_SELECTION_CRITERIA.md before enabling).
TOGETHER_API_KEY=
...
Merging to `main` triggers Vercel for the **frontend** independently. The workflow [`.github/workflows/deploy-backend-emma.yml`](.github/workflows/deploy-backend-emma.yml) SSHs to the Emma VM and runs [`scripts/deploy-runtime.sh`](scripts/deploy-runtime.sh): the VM **fetches and resets to `origin/main`**, runs **`npm ci`** (full install, including devDependencies required for migrations), **`npm run build`**, **`npm run migrate`**, **`npm run bootstrap:application-role`** (creates `application_role` when migration 021 could not — requires `DATABASE_ADMIN_URL` on the VM or via the `DATABASE_ADMIN_URL` Actions secret; the decoded admin URL is **not** exported for the rest of the deploy script so PM2 `--update-env` never inherits it), then starts or reloads PM2 from **`ecosystem.config.js`**. The final smoke test requires **`/api/health` HTTP 200** and **`status` not `down`** (no silent green deploy when core checks fail). The backend is **not** built in Actions and rsynced anymore; the VM always runs the current tree from git.

**RLS / `application_role`:** Managed Postgres often runs migrations with a user that **cannot** `CREATE ROLE`, so migration `021_rls_setup.sql` may complete without creating `application_role`. Read [`docs/RUNBOOKS/application-role-bootstrap.md`](docs/RUNBOOKS/application-role-bootstrap.md) for the privileged bootstrap, verification SQL, and the difference between `DATABASE_URL` and `DATABASE_ADMIN_URL`.
...
| `DATABASE_ADMIN_URL` | No | **Opt-in:** privileged Postgres connection string (superuser or `CREATEROLE` + DDL rights). If set, the workflow passes it to the VM as **`DATABASE_ADMIN_URL_B64`** (base64); `deploy-runtime.sh` decodes into a subshell-only env for bootstrap, then **unsets** it before PM2 (`--update-env`). GitHub masks the secret in logs. Alternatively, put `DATABASE_ADMIN_URL` only in `backend/.env` on the server and omit this secret. |
