...
The Skeptic and Verifier roles exist specifically to prevent the system from becoming a sophisticated hallucination engine.`,
  },
  {
    ...
    content: null,
    steps: [
      'Go to Research → write a specific, testable query. You do not need a pre-built corpus: when discovery is enabled, the system can search the open web and ingest bounded external sources to support the run.',
      'Optional — seed the corpus first: use Ingest to add PDFs, URLs, or text you want prioritized. On Research, use filter tags if you want retrieval scoped to tagged sources.',
