...
- **Out of scope confirmations:** `backend/src/constants/prompts.ts` — **not** modified; Rule 28 tier identifier strings — **not** renamed; `pipelineSchematicData.ts` canonical stage names (marquee schematic) — **not** modified; research inference paths — **not** modified.
