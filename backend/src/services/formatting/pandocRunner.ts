...
function docxReferenceBaseName(style: ExportStyle): string {
  // Both Chicago CSL variants share one reference DOCX (templates/README.md).
  if (style === 'chicago-author-date' || style === 'chicago-note') return 'chicago';
  return style;
}

function resolveDocxReferencePath(style: ExportStyle): string {
  return join(__dirname, 'templates', `${docxReferenceBaseName(style)}-reference.docx`);
}
