...
-- then SET NULL via UPDATE. ALTER TYPE ... USING (...) cannot contain subqueries
-- (Postgres: "cannot use subquery in transform expression"), so FK eligibility
-- is enforced before the cast, not inside USING.
...
-- Clear bad TEXT values before ALTER TYPE; USING must not reference other rows.
DO $$
BEGIN
  IF EXISTS (
    SELECT 1 FROM information_schema.columns
